"""FUTBIN news scraper (https://www.futbin.com/news)."""
from __future__ import annotations

import logging
import re
from typing import List

from ..config import Config
from ..models import NewsItem
from .base import (
    SourceFetchError,
    absolute_url,
    extract_nearby_date,
    extract_nearby_image,
    extract_summary,
    extract_title,
    fetch_soup,
)

log = logging.getLogger(__name__)

NEWS_URL = "https://www.futbin.com/news"
ARTICLE_HREF_RE = re.compile(r"/news/articles/(\d+)/")


def fetch_futbin_news(limit: int = None) -> List[NewsItem]:
    limit = limit or Config.MAX_ITEMS_PER_SOURCE_PER_RUN
    try:
        soup = fetch_soup(NEWS_URL)
    except SourceFetchError as exc:
        log.warning("FUTBIN fetch failed, skipping this run: %s", exc)
        return []

    items: List[NewsItem] = []
    seen_ids = set()

    for anchor in soup.find_all("a", href=True):
        href = anchor["href"]
        match = ARTICLE_HREF_RE.search(href)
        if not match:
            continue

        article_id = match.group(1)
        if article_id in seen_ids:
            continue

        title = extract_title(anchor)
        if not title:
            continue

        seen_ids.add(article_id)
        url = absolute_url(NEWS_URL, href)

        items.append(
            NewsItem(
                source="futbin",
                source_label="FUTBIN",
                item_id=f"futbin:{article_id}",
                title=title,
                url=url,
                published_at=extract_nearby_date(anchor),
                summary=extract_summary(anchor),
                thumbnail_url=extract_nearby_image(anchor, NEWS_URL),
            )
        )
        if len(items) >= limit:
            break

    return items
